...
int main()
{
	//cout << "Calculation f(x) started" << endl << endl;
	MSG a;
	...
	while (n < 100)
	{
		...
		{
			PostThreadMessage(ui_process_thread_id, getResult(s.str()) + 2, n, 0);
			return 0;
		}
		PostThreadMessage(ui_process_thread_id, NULL, n, 4);
		n++;
		s.clear();
	}
	cout << "Calculation f finished" << endl << endl;
	PostThreadMessage(ui_process_thread_id, NULL, n, 2);
